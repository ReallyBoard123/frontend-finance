...
import { Search, RefreshCw, ArrowUpDown, HelpCircle, FolderTree, X, Trash2 } from 'lucide-react';
...
export function TransactionList({ 
  ...
  // Direct transaction storage - simplified from server response
  ...
  // Function to fetch all transactions with types
  const fetchAllTransactions = async () => {
    try {
      ...
      // Store all transaction types directly from server response
      setAllTransactions(data.allTransactions || []);
      setRegularTransactions(data.regularTransactions || []);
      setSpecialTransactions(data.specialTransactions || []);
      setMissingTransactions(data.missingTransactions || []);
      
      console.log(`Loaded transactions: ${data.counts.all} total, ${data.counts.regular} regular, ${data.counts.special} special, ${data.counts.missing} missing`);
    } catch (error) {
  ...
  const getActiveTransactions = () => {
    switch (activeTab) {
      ...
      default:
        return allTransactions; // Use all transactions for "all" tab
    }
  };
